...
const Prescriptions = () => {
  ...
  // ✅ حساب العمر والتحقق من عمر 17 سنة
  useEffect(() => {
    if (formData.birthDate) {
      ...
      // ✅ تحقق مباشر من العمر
      if (age < 17) {
        toast.error("❌ Patient must be at least 17 years old.");
  ...
  // ✅ التحقق المباشر من التاريخ والوقت
  useEffect(() => {
    if (formData.visitDate) {
      const day = formData.visitDate.getDay();
      if (day === 5 || day === 6) {
        toast.error("❌ Visits are not allowed on Friday or Saturday.");
        document.documentElement.scrollTop = 0;
        document.body.scrollTop = 0;
      }
    }
    ...
    if (formData.visitTime) {
      ...
      if (hour < 8 || hour > 14 || (hour === 14 && minute > 0)) {
        toast.error("❌ Visit time must be between 08:00 and 14:00.");
        document.documentElement.scrollTop = 0;
        document.body.scrollTop = 0;
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // ✅ التحقق من الحقول المطلوبة دفعة واحدة
    ...
    try {
      ...
      toast.error("❌ Failed to check for duplicate prescription.");
      document.documentElement.scrollTop = 0;
      document.body.scrollTop = 0;
      return;
    }
